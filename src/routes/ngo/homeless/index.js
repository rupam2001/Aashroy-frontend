...
export default function NgoHomeLess() {
  ...
  const peopleWiseSearchAsync = async () => {
    const { homeless_person_list, msg } = await searchHomelessPeopleAsync({
      person: searchQuery,
      diameter,
    });
    if (!homeless_person_list) {
      setSearchResultTitle("no results");
      setHomelessList([]);
      return;
    }
    // setTopImages(topImages);
    setSearchResultTitle(msg);
    setHomelessList(homeless_person_list);
    setHomelessMap(homelessMap);
  };
